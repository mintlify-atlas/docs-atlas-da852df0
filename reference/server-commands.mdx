...
<ParamField path="back" type="command">
  Teleports you back to your last location.
  ...
  **Usage:**
  ...
  <Note>This command cannot be used from the console - it requires a player.</Note>
</ParamField>

### fast

<ParamField path="fast" type="command">
  Enables or disables fast cheat commands like "hatch" (instant egg hatching) or "grow" (instant plant growth).
...
<ParamField path="promote" type="command">
  Sets specific permission levels for a player.
  ...
  **Example:**
  ...
  <Warning>You cannot promote players to a permission level higher than your own.</Warning>
</ParamField>
...
<ParamField path="config" type="command">
  Opens the server configuration file in the system's default text editor.
...
<ParamField path="directory" type="command">
  Opens the current server directory in the system's file explorer.
...
<ParamField path="loadbatch" type="command">
  Loads entities at specific coordinates for debugging and testing.
...
<ParamField path="query" type="command">
  Query the entity associated with a given NitroxId for debugging.
...
<ParamField path="swapserializer" type="command">
  Swaps the server's serialization mode between JSON and ProtoBuf.
